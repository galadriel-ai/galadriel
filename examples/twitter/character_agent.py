...
class TwitterCharacterAgent(ToolCallingAgent):
    def __init__(self, character_json_path: str, **kwargs):
        ...
        try:
            ...
            _ = load_agent_template(TWITTER_SYSTEM_PROMPT, Path(self.character_json_path))
    ...
    async def execute(self, message: Message) -> Message:
        try:
            ...
            character_prompt = load_agent_template(TWITTER_SYSTEM_PROMPT, Path(self.character_json_path))
            _fill_deterministic_prompt(character_prompt, message)
...
def _fill_deterministic_prompt(prompt: str, message: Message) -> str:
    # 1. perplexity logic
    # 2. random logic
    # 3. deterministic logic
    # etc until we have the prompt filled with context
    return prompt
