...
class TwitterPostTool(TwitterClient, Tool):
    ...
    inputs = {
        ...
        "in_reply_to_id": {"type": "string", "description": "The tweet ID to respond to, empty string for NOT replying"},
    }
    output_type = "object"
    ...
    def forward(self, tweet: str, in_reply_to_id: str) -> Dict:
...
class TwitterRepliesTool(TwitterClient, Tool):
    ...
    # Hacky solution..
    def forward(self, conversation_id: str) -> str:
...
def _get_credentials_from_env() -> TwitterCredentials:
    if (
        not os.getenv("TWITTER_CONSUMER_API_KEY") or
        not os.getenv("TWITTER_CONSUMER_API_SECRET") or
        not os.getenv("TWITTER_ACCESS_TOKEN") or
        not os.getenv("TWITTER_ACCESS_TOKEN_SECRET")
    ):
        raise Exception("Missing Twitter environment variables")
